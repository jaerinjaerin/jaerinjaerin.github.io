---
title: '테스트 코드 작성 연습하기 🐻 (fireEvent)'
date: 2023-06-07
tags: ['Develop']
...
버튼에서와 동일한 방법을 사용하면 됩니다. `fireEvent.click`을 사용해 체크박스를 클릭하는 거죠.
...
현재 테스트 코드에서는 `const checkbox = screen.getByRole('checkbox');` 로 checkbox를 확인하는데요, 체크박스가 여러 개 존재하는 경우 문제가 생기게 됩니다. 이때 사용하는 것이 **getByRole의 name 옵션**입니다.
...
<Callout>
  ...
  “checkbox” and name “Disable button”
</Callout>
...
<div>
  ...
  <input
    ...
    defaultChecked={disabled}
  />
  <label htmlFor="enable-button-checkbox">Disable button</label> // 🚧 label에 동일한 id값
</div>
```
